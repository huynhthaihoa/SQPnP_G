...
def get_refined_landmarks(bbox, detection_result):
...
def draw_performance_info(img, fps, sqpnp_time=None, cpu_cycles=None):
    ...
    cv2.putText(
        img,
        f"Points: {len(landmarks3d_ids_pnp)}",
        (10, y_offset),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (255, 255, 0),
...
def main():
    ...
    output_path = "head_pose_output.mp4"
    writer = None
    ...
    while cap.isOpened():
        ...
        if face_id in bbox_dict.keys():
            ...
            if min_idx != -1:
                ...
                crop_face_image = original_frame[face_bboxes[min_idx][1] : face_bboxes[min_idx][3], face_bboxes[min_idx][0] : face_bboxes[min_idx][2], :].astype(np.uint8)
                # Setup the input image
                mp_image = Image(image_format=ImageFormat.SRGB, data=crop_face_image)   
