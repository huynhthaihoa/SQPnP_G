...
class DetectionModel(BaseModel): 
    def __init__(
        self, model_path, device, classes, apply_all_optim=True, nms_threshold=0.7, conf_threshold=0.5, use_min_ratio=True, class_agnostic=True) -> None:
    ...
    def vis(self, boxes, scores, cls_ids):#, class_names=None):
        ...
        for i in range(len(boxes)):
            ...
            x2 = int(box[2])
            y2 = int(box[3])
            if cls_id not in bboxes.keys():
                bboxes[cls_id] = list()
            bboxes[cls_id].append((x1, y1, x2, y2, score))

        return bboxes#human_bboxes, hand_on_bboxes, hand_off_bboxes, face_bboxes, phone_bboxes
    ...
    def postprocess_without_nms_block(self, dets):#, ratio):
        ...
        if dets is not None:
            ...
            if nms_dets is not None:
                ...
                bbox_dict = self.vis(final_boxes, final_scores, final_cls_inds)
    ...
    def pipeline(self, input_image):
        """Pipeline for YOLOX detection model
        Args:
        - input_image: Input image for detection.
        
        Returns:
        - bbox_dict: Dictionary containing bounding boxes for different classes.  
        """ 
        start_time = time()
        ...
        if self.with_nms_block:
            bbox_dict = self.postprocess_with_nms_block(dets.astype(np.float32))#, ratio)  # output visualization
        else:
            bbox_dict = self.postprocess_without_nms_block(dets.astype(np.float32))#, ratio)
